...
- // 1. Pointer to const data (data is const, pointer is not)
...
- // 2. const pointer (pointer is const, data is not)
...
| Task                       | Java Keyword    | C++ Operator              | Where It's Used                                                

|**Call Parent Constructor**| `super(args);`   | `ParentClass(args)` | First line of child's constructor (Java) or member initializer list (C++).|
| **Call Parent Method** | `super.methodName();`|`ParentClass::methodName();`| Anywhere inside child class methods.|                               
| **Access Parent Field**| `super.fieldName;`| `ParentClass::fieldName;`   | Anywhere inside child class methods. 

## E. enums (enumeration):
- is a user-defined data type that lets you define a set of named constant values, making code more readable, safer, and self-documenting.
...
#### 1️⃣ Primitive data types exist (MAIN reason)
...
🔹 Note:
 ...
 class B{};
 //now be can access all the members( private, public, protected) of class A;
